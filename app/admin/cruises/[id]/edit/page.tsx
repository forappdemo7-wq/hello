...
export default function EditCruise() {
  ...
  const fetchCruise = async () => {
    const token = localStorage.getItem('adminToken');
    const res = await fetch(`/api/cruises/${params.id}`, {
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="bg-gray-900 border-b border-gray-800 sticky top-0 z-10">
        <div className="container mx-auto px-6 py-4">
          <Link href="/admin/cruises" className="text-white hover:text-gray-300">← Back to Cruises</Link>
        </div>
      </div>

      <div className="container mx-auto px-6 py-8 max-w-3xl">
        <h1 className="text-3xl font-bold text-white mb-8">Edit Cruise</h1>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Cruise Name</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Duration (e.g., "7 Nights")</label>
              <input
                type="text"
          ...
          <div className="flex gap-4 pt-4">
            ...
            <Link href="/admin/cruises" className="bg-gray-700 hover:bg-gray-600 text-white px-6 py-2 rounded-lg">
              Cancel
            </Link>
          </div>
        </form>
